...
let PERSISTENCE_WRITE_IN_FLIGHT = false;
let PERSISTENCE_LATEST_SNAPSHOT = null;
...
function mergePersistentState(latest, incoming) {
  ...
  for (const key of collectionKeys) {
    ...
    const identity = item => {
      ...
      if (key === 'ranges') {
        // Range identity is the normalized canonical name/master key, not a
        // single Lamix external id. Lamix can expose the same range under
        // different ids/aliases; using externalId here allowed duplicates to
        // survive in Supabase and later reappear in the UI.
        const canonical = rangeIdentity(item);
        if (canonical) return `range:${canonical}`;
        if (item.externalId) return `range-ext:${String(item.externalId).trim().toLowerCase()}`;
      }
      if (key === 'superRateCards') return `super-rate:${keyOf(item.country, item.operator)}`;
      if (key === 'adminRateCards') return `admin-rate:${Number(item.adminId)}:${keyOf(item.country, item.operator)}`;
      if (key === 'userRangeRates') return `user-range-rate:${Number(item.userId)}:${Number(item.rangeId)}`;
      if (key === 'rangeRequests') return `range-request:${Number(item.id)}`;
    ...
    for (const item of b) {
      ...
      if (key === 'ranges') {
        // Use the same loss-averse range merge used by the local dedupe pass.
        // This makes the Supabase copy canonical as well, so duplicate ranges
        // cannot be resurrected by a later Vercel instance.
        byId.set(k, mergeRangeRecords(previous, item, { numbers: [] }));
        continue;
      }
      ...
      if (key === 'userRangeRates') {
        const pa = Date.parse(previous.updatedAt || previous.createdAt || '') || 0;
        const pb = Date.parse(item.updatedAt || item.createdAt || '') || 0;
        byId.set(k, pb >= pa ? { ...previous, ...item } : { ...item, ...previous });
        continue;
      }

      if (key === 'superRateCards' || key === 'adminRateCards') {
        const pa = Date.parse(previous.updatedAt || previous.createdAt || '') || 0;
        const pb = Date.parse(item.updatedAt || item.createdAt || '') || 0;
        byId.set(k, pb >= pa ? { ...previous, ...item } : { ...item, ...previous });
        continue;
      ...
      if (key === 'rangeRequests') {
        // A reviewed request is terminal. A stale Vercel instance must never
        // turn an approved/rejected request back into pending.
        const pa = Date.parse(previous.reviewedAt || previous.updatedAt || previous.createdAt || '') || 0;
        const pb = Date.parse(item.reviewedAt || item.updatedAt || item.createdAt || '') || 0;
        const pTerminal = previous.status === 'approved' || previous.status === 'rejected';
        const iTerminal = item.status === 'approved' || item.status === 'rejected';
        if (iTerminal && !pTerminal) byId.set(k, { ...previous, ...item });
        else if (pTerminal && !iTerminal) byId.set(k, { ...item, ...previous });
        else byId.set(k, pb >= pa ? { ...previous, ...item } : { ...item, ...previous });
        continue;
      }

      if (key === 'bulkAllocationHistory') {
        // History rows are append-only. Keep whichever copy has the richer
        // allocation result instead of allowing an old empty snapshot to win.
        const pCount = Number(previous.allocated || 0);
        const iCount = Number(item.allocated || 0);
        byId.set(k, iCount >= pCount ? { ...previous, ...item } : { ...item, ...previous });
        continue;
      }
...
function queueRemoteSave(db) {
  ...
  // Do not enqueue one Supabase request for every mutation. A Lamix inventory
  // scan can touch hundreds/thousands of records; the old FIFO queue made a
  // newly-created Agent/Client or a saved range rate wait behind hundreds of
  // stale snapshots. Coalesce all pending saves to the newest in-memory state.
  PERSISTENCE_PENDING_WRITES++;
  const snapshot = JSON.parse(JSON.stringify(db));
  const run = async () => {
    let latest = snapshot;
    while (true) {
      await remotePut(latest);
      // If another mutation happened while remotePut was in flight, persist the
      // newest snapshot too. Otherwise finish this flush with one write.
      if (PERSISTENCE_LATEST_SNAPSHOT) {
        latest = PERSISTENCE_LATEST_SNAPSHOT;
        PERSISTENCE_LATEST_SNAPSHOT = null;
        continue;
      }
      break;
    }
  };
  if (PERSISTENCE_WRITE_IN_FLIGHT) {
    PERSISTENCE_LATEST_SNAPSHOT = snapshot;
    PERSISTENCE_PENDING_WRITES = Math.max(0, PERSISTENCE_PENDING_WRITES - 1);
    return PERSISTENCE_WRITE;
  }
  PERSISTENCE_WRITE_IN_FLIGHT = true;
  PERSISTENCE_WRITE = PERSISTENCE_WRITE.catch(() => {}).then(run).catch((error) => {
    PERSISTENCE_ERROR = error;
    console.error('Persistent DB write failed:', error.message);
    throw error;
  }).finally(() => {
    PERSISTENCE_WRITE_IN_FLIGHT = false;
    PERSISTENCE_PENDING_WRITES = Math.max(0, PERSISTENCE_PENDING_WRITES - 1);
  });
  return PERSISTENCE_WRITE;
}
...
function numberBelongsToRange(number, range) {
  if (!number || !range) return false;
  const rangeId = range.id == null ? null : String(range.id);
  const exts = new Set([
    range.externalId,
    ...(Array.isArray(range.externalIds) ? range.externalIds : [])
  ].filter(v => v !== null && v !== undefined && String(v).trim() !== '').map(v => String(v)));
  return (number.rangeId != null && rangeId != null && String(number.rangeId) === rangeId) ||
    (number.carrierRangeId != null && exts.has(String(number.carrierRangeId)));
}
...
function bulkAllocateNumbers({ requester, rangeIds, targetUserIds, amountPerRange, rangeRates = {} }) {
  ...
  for (const range of ranges) {
    ...
    if (requester.role === ROLES.SUPER_ADMIN) {
      ...
      range.rate = rate;
      range.rateCustomized = true;
      range.rateUpdatedAt = new Date().toISOString();
      range.updatedAt = range.rateUpdatedAt;
      continue;
    }
  ...
  for (const range of ranges) {
    const pool = db.numbers.filter(n => {
      if (!numberBelongsToRange(n, range) || n.status !== 'available' || n.assignedClientId || n.holderUserId) return false;
      const poolOwner = n.poolOwnerId == null ? null : Number(n.poolOwnerId);
      // Super Admin owns the master inventory and may allocate any genuinely
      // free number, even if an earlier assignment had temporarily set its
      // poolOwnerId. Manager/Agent allocations remain scope-protected.
      if (requester.role === ROLES.SUPER_ADMIN) return true;
      return poolOwner === Number(requester.id);
    });
  ...
  for (const target of requestedTargets) {
    for (const range of ranges) {
      ...
      for (const n of picked) {
        if (requester.role === ROLES.SUPER_ADMIN) {
          n.poolOwnerId = target.role === ROLES.CLIENT ? Number(target.parentAdminId || target.parentManagerId || target.id) : Number(target.id);
...
function reviewRangeRequest(id, status, reviewer) {
  ...
  try {
    ...
    if (status==='approved' && (requester.role===ROLES.MANAGER || requester.role===ROLES.AGENT)) {
      ...
      if (range) {
        if (requester.role===ROLES.MANAGER) {
          range.assignedManagerId=requester.id;
          range.assignedAgentId=null;
        } else {
          range.assignedManagerId=requester.parentManagerId ? Number(requester.parentManagerId) : null;
          range.assignedAgentId=requester.id;
        }
        touchAllocation(range);
        range.updatedAt = now;

        // Approval must also transfer the currently free inventory belonging to
        // this range. Previously only the range row changed, so Bulk Add could
        // immediately see the approved range but still find zero numbers.
        const aliases = new Set([range.externalId, ...(range.externalIds || [])].filter(v => v != null).map(String));
        const ownerId = requester.role === ROLES.AGENT ? Number(requester.id) : Number(requester.id);
        for (const n of db.numbers) {
          const matches = (n.rangeId != null && Number(n.rangeId) === Number(range.id)) || (n.carrierRangeId != null && aliases.has(String(n.carrierRangeId)));
          if (!matches || n.holderUserId || n.assignedClientId || n.status !== 'available') continue;
          n.poolOwnerId = ownerId;
          n.ownerAdminId = ownerId;
          touchAllocation(n);
        }
      }
    }
