...
function save(db) {
  DB_CACHE = db;
  // Vercel serverless functions have a read-only deployed filesystem.
  // Keep the in-memory DB for the lifetime of the warm function instead of
  // crashing requests with EROFS when a session/user/range is saved.
  // NOTE: this is a compatibility fallback; persistent production storage
  // should be moved to an external database (Postgres/KV/etc.).
