...
function mergePersistentState(latest, incoming) {
  ...
  for (const key of collectionKeys) {
    ...
    const identity = item => {
      ...
      if (item.token !== undefined) return `token:${item.token}`;
      return `raw:${JSON.stringify(item)}`;
    };
    const byId = new Map(a.map(item => [identity(item), item]));
    for (const item of b) byId.set(identity(item), item);
...
async function remotePut(db) {
  // Re-read immediately before every write. This protects against the most
  // damaging Vercel race: a carrier sync (or another request) saving an older
  // snapshot after a Manager/Agent was created.
  let payload = db;
  try {
    const latest = await remoteGet();
    if (latest && typeof latest === 'object') payload = mergePersistentState(latest, db);
  } catch (error) {
    // If the read fails, do not silently replace a healthy remote state with a
    // stale snapshot. Abort the write so the persistence error is visible.
    throw new Error(`Supabase pre-write read failed: ${error.message}`);
  }
