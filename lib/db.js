...
async function remotePut(db) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    const rows = await response.json();
    if (Array.isArray(rows) && rows.length > 0) { REMOTE_UPDATED_AT = nextUpdatedAt; REMOTE_CHECK_AT = Date.now(); return; }
...
function upsertCarrierRange({ carrier = 'lamix', externalId, name, sourceName = name, country, operator, rate = 0 }) {
  ...
  if (range) {
    ...
    range = {
      id: allocateNumericId(db, 'ranges', 'nextRangeId'), carrier, externalId: ext, sourceName: sourceName || name,
