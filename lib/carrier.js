...
function findRangeForCdr(rangeValue, rangesById) {
  ...
  for (const candidate of rangesById.values()) {
    if ([candidate?.name, candidate?.sourceName, candidate?.masterKey, candidate?.externalId].some(k => normalizeRangeLookupKey(k) === wanted)) return candidate;
  }
  return null;
}
...
async function fetchCdrsFast() {
  const payload = await request('/cdrs', { limit: 250 });
  return unwrapList(payload, ['cdrs', 'cdr', 'messages', 'records', 'items', 'results', 'data']);
}
...
async function syncCdrOnly() {
  ...
  const cdrItems = await fetchCdrs();
  ...
  const rangesById = new Map(dbRanges.map(r => [String(r.externalId), r]));
...
async function runBackgroundSync() {
  ...
  if (browserAutoRunning) return { skipped: true, reason: 'background sync already running', status: getStatus() };
  browserAutoRunning = true;
  try {
    const cdr = await syncCdrOnly();
    let inventory = null;
    if (Date.now() - lastBrowserInventoryAt >= INVENTORY_POLL_INTERVAL_MS) {
      inventory = await syncInventoryOnly();
      lastBrowserInventoryAt = Date.now();
    }
    lastResult = { ...(lastResult || {}), ...cdr, inventory, backgroundBrowser: true, backgroundBrowserAt: new Date().toISOString() };
    return lastResult;
  } finally {
...
module.exports = { config: safeConfig, isConfigured, testConnection, runSync, runBackgroundSync, getStatus, start, getTestPanelData };
