...
function config() {
  ...
  return { token, baseUrl, ownerAdminId, sessionCookie, csrfToken };
}
...
async function requestSessionPath(path, query = {}) {
  ...
  const url = new URL(path, 'https://panel.lamix.org');
  for (const [key, value] of Object.entries(query)) {
  ...
  try {
    const headers = {
      Accept: '*/*',
      Referer: 'https://panel.lamix.org/test-panel',
...
module.exports = { config: safeConfig, isConfigured, testConnection, runSync, getStatus, start, getTestPanelData };
